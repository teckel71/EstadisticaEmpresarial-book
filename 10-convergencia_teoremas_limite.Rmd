...
3. Cerramos el círculo con el **esquema completo de aproximaciones** que enlaza a la binomial, la Poisson y la normal, junto con las reglas prácticas que determinan cuándo aplicar cada una.
...
Se escribe $\xi_{n} \xrightarrow{P} \xi$. Traducido a román paladino: **por grande que sea $n$, la probabilidad de que $\xi_{n}$ se aleje del límite en más que un margen fijo tiende a cero**. No es que $\xi_{n}$ sea exactamente $\xi$ para $n$ grande —de hecho, no lo será casi nunca—, pero la probabilidad de errar por mucho se hace despreciable.

### La ley (débil) de los grandes números {#lgn}

El ejemplo canónico —y el más importante en la práctica— de convergencia en probabilidad es la **ley (débil) de los grandes números**. Si $\xi_{1}, \xi_{2}, \ldots$ son variables aleatorias independientes e idénticamente distribuidas (i.i.d., abreviatura estándar) con media $\mu$ y varianza finita, entonces la **media muestral**
...
En palabras llanas: **si tomamos suficientes observaciones y calculamos su media, esa media se parecerá tanto como queramos a la verdadera media poblacional**. Es el principio que legitima usar una muestra grande para estimar un parámetro desconocido, y también el argumento subyacente cuando un casino informa a un cliente enfadado que "a la larga la casa siempre gana": no es un capricho probabilístico, es la LGN aplicada al margen del jugador.
...
costop <- read_excel("data/interestelar_300.xlsx", sheet = "Datos") |>
  pull(COSTOP)
...
**¿Qué distribución sigue $\bar{\xi}_{60}$?** La distribución exponencial es asimétrica —lo vimos en el capítulo 9— y por tanto los tiempos individuales *no* son normales. Pero por el TCL (Lindeberg–Lévy), con $n = 60$ el promedio ya se comporta aproximadamente como una normal:

$$
\bar{\xi}_{60} \;\approx\; N\!\left( 12;\; \frac{12}{\sqrt{60}} \right) \;=\; N(12;\, 1{,}549).
$$

La media es la misma que la de un tiempo individual ($12$ minutos). Pero la desviación típica, ¡solo $1{,}55$ minutos! El promedio de 60 contenedores es unas $\sqrt{60} \approx 7{,}75$ veces más *concentrado* que un contenedor individual. Es la razón matemática por la que promediar reduce el ruido: no es magia, es TCL.
...
- **Que el tiempo medio de descarga del turno sea inferior a $11$ minutos.**
  ...
  Tipificando con $\mu = 12$ y $\sigma / \sqrt{n} \approx 1{,}549$:
...
- **Probabilidad de completar entre $130$ y $150$ rutas.**
  ...
  Alrededor del $\hbox{85 \%}$ de las semanas. La franja habitual.
...
El astillero KORRIGAN opera $16$ cargueros según el fichero `astilleros_48.xlsx`. Las incidencias graves (variable `IIG`) se distribuyen razonablemente como una Poisson, con una tasa media de $4{,}0$ incidencias por nave a lo largo de la vida operativa registrada.[^korrigan-lambda] El **ingeniero Anakin Falco**, jefe de mantenimiento —a quien ya conocimos en el capítulo 9—, quiere estimar el número total de incidencias graves esperado en la flota completa durante los próximos cinco años, suponiendo constante la tasa histórica.

[^korrigan-lambda]: El fichero registra un promedio empírico de $4{,}06$ incidencias graves por nave KORRIGAN sobre la vida operativa completa (mediana $\approx 13{,}6$ años). Para un horizonte de cinco años, el número esperado por nave es proporcional al tiempo, lo que da la tasa de $1{,}5$ del enunciado.
...
- **Probabilidad de que la flota acumule menos de $20$ incidencias graves en cinco años.**
  ...
  Alrededor de un $\hbox{20 \%}$: un escenario optimista pero no desechable.
...
- **Probabilidad de que las incidencias caigan en el rango de planificación $[18;\, 30]$.**
  ...
  Alrededor del $\hbox{80 \%}$ de los escenarios. El presupuesto se dimensiona sobre este rango.
...
Recogemos ahora todas las aproximaciones vistas hasta aquí, incluidas las del capítulo 8 (binomial → Poisson), en un solo mapa. La figura \@ref(fig:fig-esquema-aprox) es el resumen visual, y la tabla \@ref(tab:tab-esquema) da los criterios operativos.
...
Cerramos el capítulo con una anticipación del papel del TCL en lo que viene, que no es poco. Prácticamente **toda** la estadística inferencial que se cursa en un grado en Economía o ADE se sostiene sobre el teorema central del límite, aunque no siempre se explicite. Tres consecuencias concretas anticipan por qué:
...
3. **La diferencia de medias muestrales de dos poblaciones es aproximadamente normal.** Combinando el TCL con la propiedad reproductiva de la normal, dos medias muestrales independientes generan una diferencia normal, con lo que se pueden construir tests de igualdad de medias. La base de la comparación estadística de dos grupos.

En los tres casos, la normal aparece como distribución de un **estimador**, no como distribución de los datos. Los datos originales pueden ser discretos, asimétricos, exóticos o directamente feos; la distribución del estimador —bajo condiciones muy generales— será la campana familiar. Es la razón por la que el resto del curso puede resolverse con una única tabla, la de la normal estándar.

## Recapitulación {#recap-cap10}

Tres ideas conviene retener de este capítulo por encima del resto:
...
3. **La agregación reduce la incertidumbre relativa.** La media muestral de $n$ observaciones tiene desviación típica $\sigma / \sqrt{n}$: si $n = 100$, el ruido se divide por diez; si $n = 10\,000$, por cien. Es la razón matemática por la que las encuestas grandes son más fiables que las pequeñas, por la que promediar reduce el error de un instrumento, y por la que las flotas grandes son operativamente más predecibles que las flotas pequeñas. La diversificación, medida en términos de riesgo, es una consecuencia numérica del TCL.
