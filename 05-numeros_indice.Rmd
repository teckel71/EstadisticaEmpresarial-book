...
arrakis %>%
  ...
  kable(
    caption   = "Ruta Arrakis–Coruscant de Arrakis Freight, 2020–2023.",
    align     = "cccc",
    booktabs  = TRUE
  ) %>%
  kable_styling(
...
Podríamos limitarnos a mirar los números en bruto, pero comparar $54$ PAVOs frente a $30$ PAVOs, o $2\,100$ frente a $1\,200$ mil toneladas, no da una idea rápida y homogénea. Lo que sí la da es medir el precio (o la cantidad, o el valor) de $2023$ como **múltiplo** del de $2020$: $54/30 = 1{,}80$. El precio ha pasado a valer $1{,}80$ veces lo que valía en $2020$; o, equivalentemente, ha subido un $80\,\%$. Ese cociente, $1{,}80$, es un **número índice**.
...
Cuando el período base es siempre el mismo, hablamos de números índice **de base fija**. Sobre nuestro ejemplo, tomando como base el año $2020$:
...
arr_idx_bf %>%
  ...
  kable(
    caption   = "Índices simples de base fija (base = 2020) para la ruta Arrakis–Coruscant.",
    align     = "cccc",
    booktabs  = TRUE,
    escape    = FALSE
  ) %>%
...
La lectura es directa. Entre $2020$ y $2023$, el precio del servicio de carga en la ruta Arrakis–Coruscant se ha multiplicado por $1{,}80$ (ha subido un $80\,\%$); la cantidad transportada, por $1{,}75$ (ha subido un $75\,\%$); y el valor facturado, por $3{,}15$ (ha subido un $215\,\%$). Y, como es fácil comprobar, $1{,}80 \times 1{,}75 = 3{,}15$: el fuerte tirón de las ventas es fruto de que precio y cantidad han crecido a la vez.
...
arr_idx_enc %>%
  ...
  kable(
    caption   = "Índices simples encadenados para la ruta Arrakis–Coruscant.",
    align     = "cccc",
    booktabs  = TRUE,
    escape    = FALSE
  ) %>%
...
La lectura ahora es distinta y complementaria: cada valor mide la variación **respecto al año inmediatamente anterior**. Un dato de $1{,}3636$ en el índice de precios de $2022$ indica que en ese año el precio subió un $36{,}36\,\%$ con respecto a $2021$. Y el índice del año $2020$ no puede calcularse: no existe $2019$ en la serie, así que el denominador falla y se representa como un guion.
...
p1 <- ggplot(arr_idx_bf, aes(Anio)) +
  ...
  labs(title    = "Índices simples de base fija (2020 = 100)",
       subtitle = "Arrakis Freight, ruta Arrakis–Coruscant",
       x = NULL, y = "Índice") +
  annotate("text", x = 2023.1, y = 315, label = "Valor",     hjust = 0, color = pal_alerta, size = 3.2) +
  annotate("text", x = 2023.1, y = 188, label = "Precio",    hjust = 0, color = pal_libro[1], size = 3.2) +
  annotate("text", x = 2023.1, y = 167, label = "Cantidad",  hjust = 0, color = pal_libro[2], size = 3.2) +
  scale_x_continuous(breaks = 2020:2023, limits = c(2020, 2024)) +
  theme_libro()
p1
```
...
**Unicidad.** Todo número índice debe **existir**. Traducido: el denominador nunca puede ser cero. Aunque parezca una obviedad, no es un caso hipotético: un producto que no se comercializara en el período base, o un componente que aún no existiera (piénsese en el precio del combustible de fusión antimateria en el año $1990$), inutilizaría cualquier intento de tomar ese período como base para ese componente.
...
Si en la ruta Arrakis–Coruscant el precio de $2023$ es $1{,}80$ veces el de $2020$, entonces, mirado en sentido inverso, el precio de $2020$ es $1/1{,}80 \approx 0{,}556$ veces el de $2023$. Coherencia elemental que un índice serio jamás debería romper.
...
hyper %>%
  ...
  kable(
    caption  = "Hyperdrive Express: precios y cantidades de dos servicios en 2020 (base) y 2023.",
    align    = "ccccc",
    booktabs = TRUE,
    escape   = FALSE
  ) %>%
...
Con estos datos, tomando $2020$ como período base y $2023$ como actual, los tres índices complejos de **precios** salen:
...
tibble(
  ...
  kable(
    caption   = "Índices complejos de Hyperdrive Express, $2020 \\rightarrow 2023$ (base $2020=100$).",
    align     = "lcc",
    booktabs  = TRUE,
    escape    = FALSE
  ) %>%
...
Sea cual sea el índice que elija el analista, la lectura es unívoca: entre $2020$ y $2023$, el conjunto de precios de los dos servicios de *Hyperdrive Express* subió alrededor de un $80\text{–}85\,\%$ y el conjunto de cantidades comercializadas alrededor de un $75\text{–}80\,\%$. Es también interesante observar la relación **valor = precio $\times$ cantidad** solo para los índices de Fisher:
...
Supongamos que disponemos de una serie de índice de precios de un conjunto de bienes de consumo galácticos con base en el año $2018$, y que queremos reexpresarla con base en $2022$. La expresión del enlace técnico es:

$$
p_{h}^{t} \;=\; \frac{p_{0}^{t}}{p_{0}^{h}},
$$

que sale directamente de aplicar la propiedad ya mencionada tomando como "base antigua" el período $0$ (aquí, $2018$) y como "base nueva" el período $h$ (aquí, $2022$). Ilustrémoslo con datos concretos.

```{r cambio-base-tabla}
cambio_base <- tibble(
  Anio   = 2018:2026,
  base18 = c(100, 130, 160, 190, 220, NA, NA, NA, NA),
  base22 = c(NA, NA, NA, NA, 100, 130, 180, 240, 250)
)

# Enlace hacia atrás usando p_h^t = p_0^t / p_0^h
p0h <- 220  # valor de la base antigua en el nuevo año base (2022)
...
cambio_base %>%
  transmute(
    `Año` = Anio,
    `Índice base 2018`  = ifelse(is.na(base18), "—", fmt(base18, 2)),
    `Índice base 2022`  = ifelse(is.na(base22), "—", fmt(base22, 2)),
    `Índice enlazado (base 2022)` = fmt(enlace22, 4)
  ) %>%
  kable(
    caption   = "Cambio de base de un índice de precios: enlace técnico $2018 \\rightarrow 2022$.",
    align     = "cccc",
    booktabs  = TRUE,
    escape    = FALSE
  ) %>%
...
$$
p_{2022}^{2018} \;=\; \frac{p_{2018}^{2018}}{p_{2018}^{2022}} \cdot 100 \;=\; \frac{100}{220}\cdot 100 \;=\; `r fmt(100/220*100, 4)`.
$$

$$
p_{2022}^{2020} \;=\; \frac{p_{2018}^{2020}}{p_{2018}^{2022}} \cdot 100 \;=\; \frac{160}{220}\cdot 100 \;=\; `r fmt(160/220*100, 4)`.
$$

El resultado se lee así: con la nueva base $2022=100$, el nivel de precios de $2018$ era $`r fmt(100/220*100, 2)`$ (es decir, un $`r fmt(100 - 100/220*100, 1)`\,\%$ inferior al de $2022$); y el de $2020$ era $`r fmt(160/220*100, 2)`$ (un $`r fmt(100 - 160/220*100, 1)`\,\%$ inferior). La serie enlazada ofrece ahora una única lectura, con la misma escala de referencia, tanto para los años antiguos como para los nuevos.
...
isla_0 %>%
  ...
  kable(
    caption = "Producción de Isla Cinturón en el año 0. Los precios están en PAVOs y las cantidades en las unidades habituales de cada bien.",
    align    = "lccc",
    booktabs = TRUE,
    escape   = FALSE
  ) %>%
...
isla_t %>%
  ...
  kable(
    caption = "Producción de Isla Cinturón en el año t. Ninguno de los cuatro precios se ha mantenido estable respecto del año 0.",
    align    = "lccc",
    booktabs = TRUE,
    escape   = FALSE
  ) %>%
...
isla_real %>%
  ...
  kable(
    caption   = "Producción del año t valorada a precios del año 0. Ahora sí se compara producción con producción.",
    align     = "lccc",
    booktabs  = TRUE,
    escape    = FALSE
  ) %>%
...
Supongamos que el *Consejo Estelar de Estadística Galáctica* publica anualmente el gasto en consumo final del Sistema Astolat (en millones de PAVOs corrientes) y, en paralelo, un **Índice General de Precios Galácticos** (IPGG) con base $2020 = 100$, que resume la evolución del nivel de precios del conjunto de bienes y servicios de consumo. Los datos publicados son:
...
serie %>%
  transmute(
    ...
    `IPGG (base 2020 = 100)` = fmt(IPGG, 2),
    `Gasto real (millones PAVOs de 2020)` = fmt(Real2020, 2)
  ) %>%
  kable(
    caption   = "Gasto en consumo final del Sistema Astolat: paso de nominal a real usando el IPGG como deflactor.",
    align     = "cccc",
    booktabs  = TRUE
  ) %>%
  kable_styling(
...
Un ejemplo de cálculo, para el año $2023$:

$$
\text{Gasto}_{2023}^{\text{real (a precios de 2020)}} \;=\; \frac{6\,300}{125{,}7}\cdot 100 \;=\; `r fmt(6300/125.7*100, 2)` \ \text{millones de PAVOs de 2020}.
$$
...
serie_long <- serie %>%
  transmute(
    ...
    `Real (PAVOs constantes de 2020)` = Real2020
  ) %>%
...
ggplot(serie_long, aes(Anio, Valor, color = Serie, shape = Serie)) +
  ...
  labs(title    = "Gasto en consumo final del Sistema Astolat",
       subtitle = "Nominal vs. real (deflactado con el IPGG, base 2020)",
