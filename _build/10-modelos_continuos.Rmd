...
![](figuras/09_continuo.jpg){.portada-img width="100%"}

::: {style="text-align: center;"}
**Figura 10.1.** [Cuando los datos se acumulan sin ponerse de acuerdo con ningún valor concreto, la campana aparece sola. No hace falta invocarla; basta con dejar de mirar demasiado cerca.]{.smallcaps}
:::
...
La figura [10.2](#fig:fig-unif-klendathu) muestra la densidad completa y sombrea la probabilidad correspondiente al intervalo central. La visualización deja claro lo que la fórmula anuncia: **calcular probabilidades en una uniforme es medir rectángulos**.

```{r fig-unif-klendathu, fig.height=3.2}
...
::: {style="text-align: center;"}
[]{#fig:fig-unif-klendathu}**Figura 10.2.** Función de densidad de $\xi \to U(0;\, 20)$: instante de aterrizaje dentro de la ventana asignada a un carguero de *arrakis freight* en el astropuerto de Klendathu. El área sombreada corresponde a $P(6 \leq \xi \leq 14) = 0{,}40$.
:::
...
La figura [10.3](#fig:fig-exp-korrigan) muestra la densidad y la probabilidad asociada al intervalo $[0{,}25;\, 0{,}75]$. Nótese la forma inconfundible: densidad máxima en el origen y decaimiento exponencial, que es lo que da nombre al modelo.

```{r fig-exp-korrigan, fig.height=3.2}
...
::: {style="text-align: center;"}
[]{#fig:fig-exp-korrigan}**Figura 10.3.** Función de densidad de $\xi \to \mathrm{Exp}(1{,}8)$: tiempo, en años, hasta la próxima avería grave de la nave *iron sovereign* (astillero KORRIGAN). El área sombreada corresponde a $P(0{,}25 \leq \xi \leq 0{,}75)$.
:::
...
La figura [10.4](#fig:fig-normal-estandar) ilustra los tres cálculos más habituales sobre la campana estándar.

```{r fig-normal-estandar, fig.height=3.4}
...
::: {style="text-align: center;"}
[]{#fig:fig-normal-estandar}**Figura 10.4.** Función de densidad de la normal estándar $\xi^{*} \to N(0; 1)$ con tres áreas de referencia: cola derecha en $z = 1{,}96$, intervalo central $[-1{,}96;\, 1{,}96]$ y cola izquierda en $z = -1{,}96$. Los tres cálculos se basan en la misma tabla de una sola cola, usando complementarios y simetría.
:::
...
La figura [10.5](#fig:fig-normal-ire) muestra la campana con los tres puntos de referencia. El resto es geometría.

```{r fig-normal-ire, fig.height=3.4}
...
::: {style="text-align: center;"}
[]{#fig:fig-normal-ire}**Figura 10.5.** Función de densidad de $\xi \to N(76{,}0;\, 8{,}2)$: IRE de una nave KORRIGAN elegida al azar. La franja sombreada corresponde al 90 % central; las áreas oscuras a las colas por debajo de 70 y por encima de 85.
:::
...
La figura [10.6](#fig:fig-var-normal) visualiza el concepto sobre la distribución de pérdidas. El VaR es el punto que deja a su derecha la probabilidad $1 - \alpha$: **el umbral que separa los días "normales" de los días "malos"**.

```{r fig-var-normal, fig.height=3.4}
mu_L <- -0.01; sd_L <- 0.15
...
::: {style="text-align: center;"}
[]{#fig:fig-var-normal}**Figura 10.6.** Distribución de pérdidas diarias de una cartera modelada como normal $L \to N(-0{,}01;\, 0{,}15)$ (millones de PAVOs). El VaR al 95 % ($\approx 237$ mil PAVOs) y al 99 % ($\approx 339$ mil PAVOs) son los cuantiles de nivel $0{,}95$ y $0{,}99$ de esta distribución: los umbrales que las pérdidas no superan salvo, respectivamente, en el 5 % y el 1 % de los días.
:::
...
::: {style="text-align: center;"}
[]{#fig:fig-aprox-normal}**Figura 10.7.** Comparación entre las probabilidades exactas de una binomial $B(50;\, 0{,}4)$ (barras) y su aproximación normal $N(20;\, \sqrt{12})$ (curva). Con $n p = 20$ y $n q = 30$ ambos claramente por encima de 5, la aproximación es muy buena en toda la masa de la distribución.
:::
...
::: {style="text-align: center;"}
[]{#fig:fig-ln-costop}**Figura 10.8.** Distribución de los costes operativos (`COSTOP`, miles de PAVOs) de las 300 empresas del fichero `interestelar_300.xlsx` con el ajuste log-normal superpuesto. El acuerdo es prácticamente perfecto: la log-normal reproduce con precisión la asimetría y el decaimiento de la cola derecha.
:::


Para hacer visible la calidad del ajuste desde otro ángulo, la tabla [10.1](#tab:tab-ln-ajuste) compara, para varios umbrales, la proporción real de empresas por debajo del umbral (calculada directamente sobre los datos) con la probabilidad que asigna el modelo log-normal. Las diferencias son inferiores a un punto porcentual en todos los casos.
...
::: {style="text-align: center;"}
[]{#tab:tab-ln-ajuste}**Tabla 10.1.** Comparación entre las probabilidades empíricas (proporción real de empresas con costes por debajo del umbral) y las que asigna el modelo log-normal ajustado. Las diferencias son inferiores a $0{,}011$ en todos los umbrales considerados.
:::
...
Todas las distribuciones vistas en este capítulo están implementadas en R con las cuatro funciones estándar `d*`, `p*`, `q*` y `r*` (densidad, distribución acumulada, cuantiles y simulación). La tabla [10.2](#tab:tab-r-cont) resume las llamadas para los cuatro modelos.
...
::: {style="text-align: center;"}
[]{#tab:tab-r-cont}**Tabla 10.2.** Funciones de R para simular los modelos continuos del capítulo. Cambiando la letra `r` inicial por `d`, `p` o `q` se accede, respectivamente, a la densidad, la función de distribución acumulada y los cuantiles.
:::
...
```{r sim-var, echo=TRUE, fig.height=3.4, out.width="80%"}
set.seed(10)
n_sim <- 10000
...
Con cuatro modelos continuos sobre la mesa, la pregunta operativa vuelve a ser: **"¿qué modelo aplico?"**. La tabla [10.3](#tab:tab-decision-cont) resume el criterio.
...
::: {style="text-align: center;"}
[]{#tab:tab-decision-cont}**Tabla 10.3.** Guía de decisión: qué modelo continuo aplicar según la naturaleza del fenómeno.
:::
