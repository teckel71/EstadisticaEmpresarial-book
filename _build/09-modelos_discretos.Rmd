...
La figura [9.2](#fig:fig-binom-arrakis) muestra la forma completa de la distribución. Concentrada en torno a la media $10{,}2$, con colas rápidamente amortiguadas, es el retrato típico de una binomial con $p$ alta.

```{r fig-binom-arrakis, fig.height=3.6}
df_bin <- tibble(
  x = 0:12,
...
::: {style="text-align: center;"}
[]{#fig:fig-binom-arrakis}**Figura 9.2.** Función de cuantía de la variable $\xi \to B(12; 0{,}85)$: número de cargueros (de 12) de *arrakis freight* que llegan a tiempo. La probabilidad se concentra en la zona alta del rango porque los cargueros suelen cumplir la ventana.
:::
...
La figura [9.3](#fig:fig-poisson-piratas) muestra la cuantía completa. Nótese la asimetría a la derecha, típica de las Poisson con $\lambda$ pequeño o moderado: es más difícil bajar de la media (limitada por el cero) que subir por encima.

```{r fig-poisson-piratas, fig.height=3.6}
df_poi <- tibble(
  x = 0:10,
...
::: {style="text-align: center;"}
[]{#fig:fig-poisson-piratas}**Figura 9.3.** Función de cuantía de $\xi \to \mathrm{Po}(2{,}3)$: avistamientos de piratas por semana en el corredor Coruscant–Naboo. La media coincide con la varianza; ambas valen $2{,}3$.
:::
...
::: {style="text-align: center;"}
[]{#tab:poisson-iig}**Tabla 9.1.** Media y varianza empíricas de la variable `IIG` (averías graves acumuladas por nave). El ratio, muy superior a 1, es un indicador claro de sobredispersión.
:::
...
La figura [9.4](#fig:fig-bn-corellian) ilustra la distribución completa. La probabilidad se concentra en valores muy bajos (0 o 1 fracasos), coherente con una $p$ alta.

```{r fig-bn-corellian, fig.height=3.6}
df_bn <- tibble(
  x = 0:10,
...
::: {style="text-align: center;"}
[]{#fig:fig-bn-corellian}**Figura 9.4.** Función de cuantía de $\xi \to \mathrm{BN}(3;\, 0{,}8)$: número de reactores defectuosos apartados antes del tercer reactor válido en la fábrica de *corellian freightworks*.
:::
...
La figura [9.5](#fig:fig-poisson-bn-iig) compara las tres cosas: la distribución empírica de `IIG`, el ajuste Poisson (deficiente) y el ajuste binomial negativa (bastante mejor).
...
La figura [9.6](#fig:fig-hyper-audit) muestra la distribución completa. Es asimétrica a la derecha porque la proporción de éxitos en la población, $p \approx 0{,}21$, es baja.
...
::: {style="text-align: center;"}
[]{#fig:fig-hyper-audit}**Figura 9.6.** Función de cuantía de $\xi \to H(300;\, 10;\, 62/300)$: número de empresas con flota renovada en una muestra aleatoria de 10 empresas auditadas.
:::
...
En la práctica se suele aceptar esta aproximación cuando $p = N_1 / N < 0{,}1$; un criterio alternativo, también habitual, es exigir que la muestra represente menos del 5–10 % de la población, esto es, $n / N < 0{,}1$. La tabla [9.2](#tab:tab-hyp-vs-bin) lo hace evidente en nuestro caso de la auditoría: con $p \approx 0{,}21$ (algo alto para la aproximación), las diferencias entre las probabilidades hipergeométricas exactas y las binomiales aproximadas son moderadas pero perceptibles.
...
::: {style="text-align: center;"}
[]{#tab:tab-hyp-vs-bin}**Tabla 9.2.** Comparación entre las probabilidades exactas hipergeométricas y su aproximación binomial en el ejemplo de la auditoría de flotas. Con $p \approx 0{,}21$ las diferencias son pequeñas pero visibles; para $p<0{,}1$ serían despreciables.
:::
...
::: {style="text-align: center;"}
[]{#tab:tab-r-funciones}**Tabla 9.3.** Funciones de R para simular los modelos discretos del capítulo. Cambiando la letra `r` inicial por `d`, `p` o `q` se accede, respectivamente, a la función de cuantía, la de distribución acumulada y los cuantiles.
:::


La figura [9.7](#fig:fig-sim-cuatro) simula 5.000 realizaciones de cada uno de los cuatro modelos "de conteo" más importantes del capítulo, aplicados a los escenarios que hemos ido presentando, y compara el histograma empírico con las probabilidades teóricas. La coincidencia es —como debe— casi perfecta.
...
::: {style="text-align: center;"}
[]{#fig:fig-sim-cuatro}**Figura 9.7.** Simulación de 5.000 realizaciones de cada uno de los cuatro modelos discretos principales del capítulo. La línea roja muestra la probabilidad teórica; las barras azules, la frecuencia relativa observada. La Ley de los Grandes Números en pleno funcionamiento.
:::
...
Con seis modelos sobre la mesa, la pregunta operativa que un lector debe hacerse cuando se enfrenta a un problema real es: **"¿qué modelo aplico?"**. La tabla [9.4](#tab:tab-decision) responde a esta pregunta con un pequeño árbol de decisión textual.
...
::: {style="text-align: center;"}
[]{#tab:tab-decision}**Tabla 9.4.** Guía de decisión: qué modelo discreto aplicar según la naturaleza del experimento.
:::
