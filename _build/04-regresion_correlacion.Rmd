...
::: {style="text-align: center;"}
**Figura 4.1.** [Cuando la relación existe, toca ponerle números: intensidad y forma.]{.smallcaps}
:::
...
### Regresión simple, regresión múltiple y tipos. {#regresion-tipos}
...
Hay además una distinción clásica en la literatura entre dos tipos de regresión, según qué se persiga con el ajuste:

-   **Regresión tipo I.** Se busca la función matemática que mejor represente la nube de puntos observada, sin ninguna pretensión probabilística. La regresión es una **descripción**: un resumen gráfico y numérico de la relación entre las dos variables.
-   **Regresión tipo II.** Se asume un modelo probabilístico subyacente —que los datos vienen de una población en la que $Y$ y $X$ se relacionan de cierta forma más la aleatoriedad del muestreo— y se estima ese modelo. La regresión se convierte en una herramienta de **inferencia**: sirve para extraer conclusiones sobre la población y no sólo sobre los datos que tenemos delante.

En este capítulo trabajaremos exclusivamente con regresión de tipo I. La versión inferencial aparecerá, en el bloque de Inferencia y Econometría, cuando dispongamos de las herramientas probabilísticas necesarias.
...
::: {style="text-align: center;"}
**Tabla 4.1.** Tabla de correlación de antigüedad ($X$) y tonelaje ($Y$) con las columnas y filas auxiliares para el cálculo directo de los momentos.
:::
...
::: {style="text-align: center;"}
**Figura 4.2.** Diagrama de dispersión de la flota y recta ajustada por mínimos cuadrados.
:::
...
::: {style="text-align: center;"}
**Tabla 4.1.** :: {style="text-align: center;"}
:::

**Tabla 4.2.** Escala orientativa para $|R|$ en el análisis descriptivo.
:::
...
::: {style="text-align: center;"}
**Figura 4.3.** Nube de puntos y recta de regresión sobre las 300 compañías de la muestra R-Stars.
:::
...
data.frame(Operaciones = xs, Tiempo = ys) %>%
  kable(col.names = c("Operaciones acumuladas ($x_i$)",
...
::: {style="text-align: center;"}
**Tabla 4.3.** Curva de aprendizaje de un tripulante en el módulo de revisión de motores de Hyperdrive Express.
:::
...
::: {style="text-align: center;"}
**Figura 4.4.** Datos de la Tabla 4.3 con dos ajustes: lineal (gris, discontinuo) y potencial (negro, continuo).
:::
...
La motivación práctica, sin embargo, se comprende de inmediato con el ejemplo de la flota. Habíamos visto que la antigüedad, por sí sola, explicaba un $`r fmt(100*R2, 0)`\,\%$ de la variabilidad del tonelaje: no está mal, pero deja mucho fuera. Si añadimos al modelo la **identidad del astillero** —una variable categórica que podríamos codificar como *dummies*— es plausible que el $R^{2}$ crezca sustancialmente, porque en el gráfico de la Figura 4.2 se percibe con claridad que cada astillero ocupa una franja de tonelajes distinta. Y si añadimos, además, el **indicador de rendimiento energético** (IRE), el modelo podría aún mejorar.

De manera más general, en la muestra de $300$ compañías R-Stars uno puede plantear regresiones múltiples sofisticadas para explicar, por ejemplo, los costes operativos en función de los ingresos, del número de empleados, de la flota, del número de rutas y del apalancamiento; o la rentabilidad económica en función de la digitalización, la diversificación, la fidelización y el tamaño de la empresa. Todo lo que hemos aprendido en este capítulo —el criterio de estimación, la descomposición de la varianza, el coeficiente de determinación— tiene su análogo natural en el caso múltiple.

Un aviso final: pasar de una a varias variables explicativas no es solo una cuestión técnica. Aparecen problemas nuevos —correlaciones altas entre las propias explicativas (la temida *multicolinealidad*), interpretaciones parciales que exigen fijar mentalmente el resto de variables, riesgo de sobreajuste cuando $p$ crece— que la regresión simple no plantea. La regresión múltiple es una herramienta enormemente potente y merecerá, en su momento, capítulo propio.
...
-   La estimación de los parámetros se realiza por **mínimos cuadrados**: se eligen los valores que minimizan $\sum e_{ij}^{\,2}\, n_{ij}$.
...
-   La **bondad del ajuste** se mide con el **coeficiente de determinación** $R^{2} = S_{\hat{y}}^{\,2}/S_y^{\,2} = 1 - S_e^{\,2}/S_y^{\,2}$, comprendido entre $0$ y $1$ y leído como proporción de varianza de $Y$ recogida por la regresión.
...
-   La **regresión múltiple** generaliza el planteamiento cuando hay varias variables explicativas. Su tratamiento riguroso —maquinaria matricial y problemas específicos como la multicolinealidad— corresponde a un curso de Econometría.
